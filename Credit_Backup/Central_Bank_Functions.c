...
int Central_Bank_monetary_policy()
{
	...
	//4.2.2010: Added ECB_INTEREST_RATE constant, for policy experiments
...
int Central_Bank_read_fiat_money_requests()
{
    ...
    //Sander: 29.07.2010: Cleanup of old code: Gov bond selling to ECB through bond market
...
int Central_Bank_read_account_update()
{
	...
	CASH += interest;
        ...
        reset_account_item_array(&GOVERNMENT_NATIONAL_ACCOUNTS);
    
    START_GOV_TO_CENTRAL_BANK_ACCOUNT_UPDATE_MESSAGE_LOOP

    add_account_item(&GOVERNMENT_NATIONAL_ACCOUNTS, gov_to_central_bank_account_update_message->id, gov_to_central_bank_account_update_message->gdp);
...
*/
		...
		*/
		...
		if(CASH>0.0)
		{
			...
			for(int i=0; i < GOVERNMENT_NATIONAL_ACCOUNTS.size;i++){

				//Payment account = gdp in adt named GOVERNMENT_NATIONAL_ACCOUNTS
				gdp += GOVERNMENT_NATIONAL_ACCOUNTS.array[i].payment_account;
			...
			for(int i=0; i < GOVERNMENT_NATIONAL_ACCOUNTS.size;i++){
				if(gdp>0)
					add_ecb_dividend_message(GOVERNMENT_NATIONAL_ACCOUNTS.array[i].id,GOVERNMENT_NATIONAL_ACCOUNTS.array[i].payment_account/gdp);
				else
					add_ecb_dividend_message(GOVERNMENT_NATIONAL_ACCOUNTS.array[i].id, 0);
			}
			//Recompute stocks
