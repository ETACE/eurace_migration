...
#include "../Labour_UNIBI/Labour_aux_headers.h"
...
int Firm_calc_input_demands_2()
{
	...
	if(feasible_output >= PLANNED_PRODUCTION_QUANTITY)
	{
		...
		if(EMPLOYEES_NEEDED_PRODUCTION<1)
					EMPLOYEES_NEEDED_PRODUCTION=1;
	...
	PLANNED_PRODUCTION_COSTS = max(EMPLOYEES_NEEDED_PRODUCTION- NO_EMPLOYEES_PRODUCTION, 0)*WAGE_OFFER*MEAN_SPECIFIC_SKILLS + min(EMPLOYEES_NEEDED_PRODUCTION, NO_EMPLOYEES_PRODUCTION)*MEAN_WAGE*1.04  + DEMAND_CAPITAL_STOCK*price_of_vintage(VINTAGE_SELECTED, INVESTMENT_SUBSIDY_TYPE, TECHNOLOGY_VINTAGES , SUBSIDY_PCT,TECHNOLOGICAL_FRONTIER);;

		//PLANNED_PRODUCTION_COSTS = EMPLOYEES_NEEDED_PRODUCTION*(1.04)*MEAN_WAGE + DEMAND_CAPITAL_STOCK*price_of_vintage(VINTAGE_SELECTED, INVESTMENT_SUBSIDY_TYPE, TECHNOLOGY_VINTAGES , SUBSIDY_PCT,TECHNOLOGICAL_FRONTIER);
...
double Firm_compute_expected_regional_market_share_aux_function(int region_id){
	...
	for(j=0; j< QUALITY_PRICE_COMPETITORS.size; j++)
	{
		...
		if(region_id == QUALITY_PRICE_COMPETITORS.array[j].region_id)
			denominator += exp(ESTIMATED_COEFFICIENTS.price*log(QUALITY_PRICE_COMPETITORS.array[j].price) + ESTIMATED_COEFFICIENTS.quality*log(2));
		else
			denominator += exp(ESTIMATED_COEFFICIENTS.price*log(QUALITY_PRICE_COMPETITORS.array[j].price) + ESTIMATED_COEFFICIENTS.quality*log(1));
	}


	if(region_id == REGION_ID)
		market_share =  exp(ESTIMATED_COEFFICIENTS.price*log(PRICE) + ESTIMATED_COEFFICIENTS.quality*log(2))/(exp(ESTIMATED_COEFFICIENTS.price*log(PRICE) + ESTIMATED_COEFFICIENTS.quality*log(2))+ denominator);
	else
		market_share =  exp(ESTIMATED_COEFFICIENTS.price*log(PRICE) + ESTIMATED_COEFFICIENTS.quality*log(1))/(exp(ESTIMATED_COEFFICIENTS.price*log(PRICE) + ESTIMATED_COEFFICIENTS.quality*log(1))+ denominator);
...
int Firm_calc_production_quantity()
{
	...
	//Ad hock: test for starting with a psoitive stock; introduced 21.12.15
	if(DAY<21){

		//PRODUCTION_QUANTITY = PRODUCTION_QUANTITY + 8.0;
		COMBINED_VARIANCE_DEMAND_FUNCTION = 180;
	...
	/*Set the total buffer depending on the service degree and the variance of the demand estimation*/
	TOTAL_BUFFER = 0;
	...
	for(i=0;i<LAST_MARKET_SIZES.size;i++)
	{
		...
		{
			if(CURRENT_MALL_STOCKS.array[j].mall_id==LAST_MARKET_SIZES.array[i].mall_id)
			{


				double market_share =Firm_compute_expected_regional_market_share_aux_function(LAST_MARKET_SIZES.array[i].region_id);
				double variance = market_share*(1-market_share);


				double current_local_market_size = 0.0;

				for(int s=0; s < LAST_MARKET_SIZES.array[i].market_sizes.size;s++){


					if(LAST_MARKET_SIZES.array[i].market_sizes.array[s].period==1){

						current_local_market_size = LAST_MARKET_SIZES.array[i].market_sizes.array[s].market_size;
						break;
				...
				//double combined_local_variance  = variance* LINEAR_REGRESSION_MARKET_SIZE.array[i].variance  +   pow(market_share,2) *LINEAR_REGRESSION_MARKET_SIZE.array[i].variance  + pow(LINEAR_REGRESSION_MARKET_SIZE.array[i].intercept,2)*variance;

				double combined_local_variance = current_local_market_size*variance;

					local_buffer =  QUANTIL_NORMAL_DISTRIBUTION*pow(combined_local_variance ,0.5);

				/*Distribute the buffer among the malls*/
				TOTAL_BUFFER += local_buffer;

				if(DAY < 21){

					//CURRENT_MALL_STOCKS.array[j].current_stock = local_buffer;


				}

				ESTIMATED_MARKET_SHARE += Firm_compute_expected_regional_market_share_aux_function(LAST_MARKET_SIZES.array[i].region_id)*(current_local_market_size)/ (MARKET_SIZE);



				/*Cumpute for each mall the critical stock*/
				 CURRENT_MALL_STOCKS.array[j].critical_stock = Firm_compute_expected_regional_market_share_aux_function(LAST_MARKET_SIZES.array[i].region_id) *(current_local_market_size)
				 + local_buffer;
      			...
      			if (PRINT_DEBUG_PRODUCTION)
     		    {
					if(ID==ID_DEBUG_PROBE)
      			    {
					   printf("\n CURFirm_compute_expected_regional_market_share_aux_functionRENT_MALL_STOCKS.array[i].critical_stock %f \t LINEAR_REGRESSION_MARKET_SIZE.array[j].intercept %f \t LINEAR_REGRESSION_MARKET_SIZE.array[j].regressor %f \t COUNTER_MONTH_SINCE_LAST_PRICE_SETTING %d \t LINEAR_REGRESSION_MARKET_SIZE.array[j].variance %f, 	Buffer:  %f    ESTIMATED_MARKET_SHARE \n ",CURRENT_MALL_STOCKS.array[j].critical_stock, LINEAR_REGRESSION_MARKET_SIZE.array[i].intercept,LINEAR_REGRESSION_MARKET_SIZE.array[i].regressor,  COUNTER_MONTH_SINCE_LAST_PRICE_SETTING,LINEAR_REGRESSION_MARKET_SIZE.array[i].variance, local_buffer, ESTIMATED_MARKET_SHARE);
		        	}
				}
 				 #endif
	...
	EXPECTED_SOLD_QUANTITY_PER_MONTH = ESTIMATED_MARKET_SHARE *MARKET_SIZE;
...
int Firm_calc_production_quantity_XXX()  // Old: this one uses the estimated market sizes; changed 04.03.2016
{
	...
	for(i=0;i<LINEAR_REGRESSION_MARKET_SIZE.size;i++)
	{
		for(j=0;j<CURRENT_MALL_STOCKS.size;j++)
		{
			if(CURRENT_MALL_STOCKS.array[j].mall_id==LINEAR_REGRESSION_MARKET_SIZE.array[i].mall_id)
			{
				...
				double market_share =Firm_compute_expected_regional_market_share_aux_function(LINEAR_REGRESSION_MARKET_SIZE.array[i].region_id);
				double variance = market_share*(1-market_share);
				...
				double combined_local_variance = LINEAR_REGRESSION_MARKET_SIZE.array[i].intercept*variance;
				...
				ESTIMATED_MARKET_SHARE += Firm_compute_expected_regional_market_share_aux_function(LINEAR_REGRESSION_MARKET_SIZE.array[i].region_id)*(LINEAR_REGRESSION_MARKET_SIZE.array[i].intercept
						 + (1+LAST_MARKET_SIZES.array[0].market_sizes.size +COUNTER_MONTH_SINCE_LAST_PRICE_SETTING)*LINEAR_REGRESSION_MARKET_SIZE.array[i].regressor)/ (LINEAR_REGRESSION_MARKET_SIZE.array[0].intercept
								 + (1+LAST_MARKET_SIZES.array[0].market_sizes.size +COUNTER_MONTH_SINCE_LAST_PRICE_SETTING)*LINEAR_REGRESSION_MARKET_SIZE.array[0].regressor);



				/*Cumpute for each mall the critical stock*/
				 CURRENT_MALL_STOCKS.array[j].critical_stock = Firm_compute_expected_regional_market_share_aux_function(LINEAR_REGRESSION_MARKET_SIZE.array[i].region_id) *(LINEAR_REGRESSION_MARKET_SIZE.array[i].intercept
...
int Firm_calc_input_demands()
{
	...
	{	
		...
		}else
		{
			...
			if(DEMAND_CAPITAL_STOCK > INV_INERTIA*TOTAL_CAPITAL_DEPRECIATION_UNITS && DEMAND_CAPITAL_STOCK > 1.0)
			{
	...
	//printf("capital_good_delivery_message->capital_good_delivery_volume %f \n",capital_good_delivery_message->capital_good_delivery_volume);
		...
		/*Distinction of cases: if subsidies for best practice technology are switched on and the purchased vintage is the best vintage available:*/
		   	...
		   	CAPITAL_COSTS += capital_good_delivery_message
        		->capital_good_delivery_volume* price_of_vintage(VINTAGE_SELECTED, INVESTMENT_SUBSIDY_TYPE, TECHNOLOGY_VINTAGES , SUBSIDY_PCT,TECHNOLOGICAL_FRONTIER);


		   	SUBSIDY_PAYMENT =   capital_good_delivery_message->capital_good_delivery_volume*(capital_good_delivery_message->capital_good_price-price_of_vintage(VINTAGE_SELECTED, INVESTMENT_SUBSIDY_TYPE, TECHNOLOGY_VINTAGES , SUBSIDY_PCT,TECHNOLOGICAL_FRONTIER));

		   	if(SUBSIDY_PAYMENT<1e-4)
		   		SUBSIDY_PAYMENT = 0.0;
...
for(i=CAPITAL_STOCK_VINTAGES.size-1;i>=0;i--)
	...
	USED_CAPITAL_UNITS = sum_already_used_vintages;
...
int Firm_calc_pay_costs()
{
    ...
    int i;
        ...
        if(OUTPUT>0.0)
        	LABOUR_COSTS_PER_UNIT = LABOUR_COSTS / OUTPUT;
        else
        	LABOUR_COSTS_PER_UNIT = 0.0;
...
{   
    	...
    	//PRODUCTION_QUANTITY = PRODUCTION_QUANTITY + 2.0;
        ...
        for(i = 0; i < PLANNED_DELIVERY_VOLUME.size; i++)
        {
            ...
            for(j = 0; j < DELIVERY_VOLUME.size; j++)
            {
                ...
                PLANNED_DELIVERY_VOLUME.array[i].mall_id)
                {
                	...
                	double add_prod=0;
                	if(DAY<21){
                	for(k=0;k<CURRENT_MALL_STOCKS.size;k++)
                			{
                		 if(DELIVERY_VOLUME.array[j].mall_id ==
                				 CURRENT_MALL_STOCKS.array[k].mall_id){
                    ...
                    DELIVERY_VOLUME.array[j].quantity+ add_prod,QUALITY,PRICE,PRICE_LAST_MONTH);
