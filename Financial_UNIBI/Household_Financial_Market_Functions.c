...
/*
    ...
    BELIEFS.last_price
    BELIEFS.fraction_to_invest

    //Step 2b: set proportion of portfolio_budget to invest in the index. This is the net investment, the remainder stays in deposits.
    if(BELIEFS.expected_volatility>0)
	proportion_to_invest = (BELIEFS.expected_return - RISK_FREE_RATE)/BELIEFS.expected_volatility;
    else
	printf("Error in Household_revises_expected_portfolio: BELIEFS.expected_volatility = %f", BELIEFS.expected_volatility);
*/
