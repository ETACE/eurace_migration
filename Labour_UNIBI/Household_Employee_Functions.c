...
int Household_receive_wage()
{
    ...
    START_WAGE_PAYMENT_MESSAGE_LOOP
        ...
        MONTHLY_INTEREST_INCOME + TRANSFER_PAYMENT + SUBSIDY_PAYMENT;
...
int Household_UNEMPLOYED_read_job_vacancies_and_send_applications()
{
	...
	int num_applications = NUMBER_APPLICATIONS;
	HH_APPLICATIONS_PER_DAY = APPLICATIONS_PER_DAY;

		if(LABORMARKET_SCENARIO_REG2 && REGION_ID==1){

			num_applications = 3;
			HH_APPLICATIONS_PER_DAY = 1;
		}
	...
	int p = round_double_to_int(100*((double)num_applications/(double)(MONTH*HH_APPLICATIONS_PER_DAY)));
	...
	if(SEARCH_TODAY == 1)
	{
					...
					else
					{
						...
						for(i = 0;i < NEIGHBORING_REGION_IDS.size; i++)
						{
							...
							if(vacancies_message->region_id == NEIGHBORING_REGION_IDS.array[i])
							{
								...
								if((wage_offer *(1- REGION_COST)) >= WAGE_RESERVATION)
								{
									/*Add vacancy to vacancy list (possible job candidates).*/
									add_vacancy(&vacancy_list,
									vacancies_message->firm_id,
									vacancies_message->region_id,
									(wage_offer *(1- REGION_COST)),MIGRATION_STATUS);
		...
		/*If there are more vacancies on the list than the maximum number of applications the household can send.*/
			if(vacancy_list.size > HH_APPLICATIONS_PER_DAY)
			{
		    	...
		    	while(vacancy_list.size > HH_APPLICATIONS_PER_DAY)
		    	{
...
int Household_read_job_offers_send_response()
{
    ...
    if(job_offer_message->job_type==0){
        ...
        if(job_offer_message->worker_id == ID)
        {
            ...
            if(REGION_ID == job_offer_message->region_id)
            {
                ...
                job_offer_message->wage_offer,0);
            }
            ...
            else
            {
                ...
                job_offer_message->firm_id, 
                job_offer_message->region_id,
                (job_offer_message->wage_offer*(1- REGION_COST)),0);
    ...
    if(job_offer_list.size > 0)
    {
        ...
        if(REGION_ID == job_offer_list.array[0].region_id)
        {
            ...
            WAGE_RESERVATION = max(UNEMPLOYMENT_PAYMENT,WAGE);
        ...
        {   
            ...
            WAGE = (job_offer_list.array[0].wage_offer/(1- REGION_COST));
            WAGE_RESERVATION = max(WAGE *(1- REGION_COST),UNEMPLOYMENT_PAYMENT);
        }
    }
...
int Household_read_application_rejection_update_wage_reservation()
{
	if(SEARCH_TODAY == 1)
	{
		...
		if(WAGE_RESERVATION < UNEMPLOYMENT_PAYMENT)
		{
		    WAGE_RESERVATION = UNEMPLOYMENT_PAYMENT;
		}
	}    
    return 0;
}
...
int Household_read_job_offers_send_response_2()
{
    ...
    if(job_offer2_message->job_type==0){
        /* Read job offer messages for this Household*/
        if(job_offer2_message->worker_id == ID)
        {
            ...
            if(REGION_ID == job_offer2_message->region_id)
            {
                ...
                job_offer2_message->wage_offer,0);
            }
            /*Job offers of firms in different regions: Household has to bear commuting costs.*/
            else
            {
                ...
                job_offer2_message->firm_id,
                job_offer2_message->region_id,
                (job_offer2_message->wage_offer *(1- REGION_COST)),0);
            }
        }
    }else{
    ...
    if(job_offer_list.size > 0)
    {
        ...
        if(REGION_ID == job_offer_list.array[0].region_id)
        {
            /*Add the new wage and sets the reservation wage to the new wage.*/
            WAGE = job_offer_list.array[0].wage_offer;
            WAGE_RESERVATION = max(WAGE,UNEMPLOYMENT_PAYMENT) ;
        }
         /*If household and firm are not in the same region.*/
        else
        {  
...
int Household_read_application_rejection_update_wage_reservation_2()
{
	if(SEARCH_TODAY == 1)
	{
		...
		/*Moved to Household_send_unemployment_benefit_notification */

		/*Updates the reservation wage by a certain fraction according to the parameter WAGE_RESERVATION_UPDATE.*/


		/*if(FLEXIBILITY_POLICY==1 && REGION_ID==2)
		{

			WAGE_RESERVATION = WAGE_RESERVATION-WAGE_RESERVATION*0.25;
...
int Household_send_unemployment_benefit_notification()
{
    ...
    //if unemployment benefit is larger than the mean wage:
        if(LAST_LABOUR_INCOME*(1-TAX_RATE_HH_LABOUR)*UNEMPLOYMENT_BENEFIT_PCT > BASIC_SECURITY_BENEFITS )
        {       
            UNEMPLOYMENT_PAYMENT = LAST_LABOUR_INCOME*(1-TAX_RATE_HH_LABOUR)*UNEMPLOYMENT_BENEFIT_PCT;
        }
        else    
        {
            UNEMPLOYMENT_PAYMENT =  BASIC_SECURITY_BENEFITS;
        }
    ...
    TOTAL_INCOME=  UNEMPLOYMENT_PAYMENT + CUM_TOTAL_DIVIDENDS + MONTHLY_INTEREST_INCOME +  TRANSFER_PAYMENT + SUBSIDY_PAYMENT;
...
int Household_send_subsidy_notification()
{
    ...
    if(MIGRATION_STATUS && POLICY_MIGRATION_TOP_UP_BENEFITS && EMPLOYEE_FIRM_ID!= -1){


		SUBSIDY_PAYMENT +=  max( MINIMUM_WAGE - WAGE,0);
...
int Household_send_tax_payment()
{
   ...
   // #endif
    ...
    if (DAY_OF_MONTH_RECEIVE_BENEFIT != DAY_OF_MONTH_RECEIVE_INCOME )
    {
        ...
        restitution_payment += ((DAY_OF_MONTH_RECEIVE_BENEFIT +
       		(20-DAY_OF_MONTH_RECEIVE_INCOME)%20)/20.0)* TRANSFER_PAYMENT;
