...
int Firm_compute_income_statement()
{
   ...
   // #endif
    ...
    EARNINGS = CUM_REVENUE - TOTAL_INTEREST_PAYMENT - CALC_PRODUCTION_COSTS - LABOUR_COSTS_INNOVATION;
...
int Firm_compute_dividends()
{
	...
	printf("average_revenue %f\n",average_revenue);


	if(PAYMENT_ACCOUNT < CONST_DIVIDEND_TRESHOLD_FULL_PAYOUT*average_revenue)	
	{
		...
		//	printf("TOTAL_DIVIDEND_PAYMENT %f = CONST_DIVIDEND_EARNINGS_RATIO %f * NET_EARNINGS  %f\n",TOTAL_DIVIDEND_PAYMENT,CONST_DIVIDEND_EARNINGS_RATIO , NET_EARNINGS);
		

	}else
	{
		TOTAL_DIVIDEND_PAYMENT = NET_EARNINGS;

		//	printf("Case 2: PAYMENT_ACCOUNT %f > CONST_DIVIDEND_TRESHOLD_FULL_PAYOUT*average_revenue %f\n",PAYMENT_ACCOUNT, CONST_DIVIDEND_TRESHOLD_FULL_PAYOUT*average_revenue );

		//	printf("TOTAL_DIVIDEND_PAYMENT %f = 1.0 * NET_EARNINGS  %f\n",TOTAL_DIVIDEND_PAYMENT, NET_EARNINGS);
...
int Firm_bankruptcy_insolvency_procedure()
{
    ...
    if(PRINT_DEBUG_BANKRUPTCY)
    {
        printf("\n\n IT %d Firm_bankruptcy_insolvency_procedure: ID %d", DAY, ID);
...
{   
    ...
    //Effect on credit market: write off debt
		...
		if(PRINT_DEBUG_BANKRUPTCY)
		{
			if(TOTAL_DEBT<1e-7)
			{
				fprintf(stderr, "\n\n IT %d Firm_bankruptcy_illiquidity_procedure: ID %d", DAY, ID);
	...
	if ((BANKRUPTCY_IDLE_COUNTER == CONST_BANKRUPTCY_IDLE_PERIOD - 1)&&(TOTAL_DEBT>1e-7))
	{
		...
		//26.7.2011: For illiquidity: check that total_debt does not increase due to debt rescaling DEBT_RESCALING_FACTOR*TOTAL_ASSETS
		if (target_debt>TOTAL_DEBT)
		{
		    fprintf(stderr,"\n\n IT %d Firm_bankruptcy_illiquidity_procedure: ID %d sets target_debt>TOTAL_DEBT %f %f", DAY, ID, target_debt, TOTAL_DEBT); 
		    target_debt=TOTAL_DEBT;
		}
    ...
    //Effect on consumption goods market
    //Option 1: all local inventory stock is lost --> see Firm_set_bankruptcy functions
    //Option 2: send back local inventory stock to factory
    //Option 3: fire-sales at local outlet malls at discount prices
...
{   
    BANKRUPTCY_IDLE_COUNTER -= 1;
		...
		if(ID==76)
	printf("Firm_bankruptcy_idle_counter()\n");
...
int Firm_reset_bankruptcy_flags()
{
   ...
   double production_quantity, production_quantity_per_mall;
    ...
    if(CURRENT_PRICE_INDEX>0){
    	PRICE = CURRENT_PRICE_INDEX;				 //DOUBLE
    	PRICE_LAST_MONTH = CURRENT_PRICE_INDEX;				 //DOUBLE
    }
