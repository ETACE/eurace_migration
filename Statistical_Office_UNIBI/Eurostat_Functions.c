...
int Eurostat_initialization()
{
    ...
    if(PRICE_QUALITY_COMBINATION.size>0){


    for(int j=0; j<PRICE_QUALITY_COMBINATION.size;j++)
    	{
    	PRICE_QUALITY_COMBINATION.array[j].price = INITIAL_PRICE;
    	}
    	...
    	for(int i=0; i < 400; i++){
    		...
    		add_price_quality_pair(&PRICE_QUALITY_COMBINATION, 1, INITIAL_PRICE,1.0 );
    	...
    	for(int i=0; i < 40; i++){
    		...
    		add_price_quality_pair(&PRICE_QUALITY_COMBINATION, 2, INITIAL_PRICE,1.0 );
...
int Eurostat_send_data_to_government()
{
    ...
    for (i=0; i<TOTAL_REGIONS; i++)
    {
        ...
        double cons_quota_rel_reg1;

        if(REGION_HOUSEHOLD_DATA.array[i].no_households>0 && REGION_HOUSEHOLD_DATA.array[0].no_households && REGION_HOUSEHOLD_DATA.array[0].consumption_budget>0)
        	cons_quota_rel_reg1=(REGION_HOUSEHOLD_DATA.array[i].consumption_budget/(1.0*REGION_HOUSEHOLD_DATA.array[i].no_households))/(REGION_HOUSEHOLD_DATA.array[0].consumption_budget/(1.0*REGION_HOUSEHOLD_DATA.array[0].no_households));
        else
        	cons_quota_rel_reg1= 1.0;

        double cons_budget_per_capita = 0.0;

             if(REGION_HOUSEHOLD_DATA.array[i].no_households>0){


             	cons_budget_per_capita = REGION_HOUSEHOLD_DATA.array[i].consumption_budget/(1.0*REGION_HOUSEHOLD_DATA.array[i].no_households);

             }

             printf("\n Region %d GDP=%2.2f\n", region, gdp);
             add_data_for_government_message(region, gdp, HISTORY_QUARTERLY[0].region_data.array[i].average_wage,HISTORY_QUARTERLY[0].region_data.array[i].unemployment_rate, cons_quota_rel_reg1,  cons_budget_per_capita);
...
int Eurostat_send_data()
{
	...
	add_msg_market_sizes_message(0, TOTAL_CONSUMPTION_BUDGET,GOVERNMENT_CONSUMPTION);

	for(int j=0; j<PRICE_QUALITY_COMBINATION.size;j++)
	{
		add_quality_price_message(PRICE_QUALITY_COMBINATION.array[j].region_id, PRICE_QUALITY_COMBINATION.array[j].quality, PRICE_QUALITY_COMBINATION.array[j].price);
	}
    ...
    for(i = 0; i < REGION_HOUSEHOLD_DATA.size; i++)
    {
    	...
    	double temp = 1.0;
    		...
    		if(REGION_FIRM_DATA.array[i].posted_vacancies!=0)
    			temp = 1-(double) REGION_FIRM_DATA.array[i].vacancies/REGION_FIRM_DATA.array[i].posted_vacancies;
	...
	REGION_FIRM_DATA.array[i].average_wage, temp,
	HISTORY_MONTHLY[0].region_data.array[i].output/HISTORY_MONTHLY[0].region_data.array[i].no_active_firms,
        CPI,PRICE_INDEX,NO_ACTIVE_FIRMS,FIRM_AVERAGE_QUALITY,FIRM_AVERAGE_PRICE,
        REGION_FIRM_DATA.array[i].wage_offer, REGION_FIRM_DATA.array[i].technology);

	//send message about the regional market size
	add_msg_market_sizes_message(REGION_HOUSEHOLD_DATA.array[i].region_id,  REGION_HOUSEHOLD_DATA.array[i].consumption_budget, REGION_GOVERNMENT_DATA.array[i].consumption_expenditures );
	...
	if(SUBSIDY_REDISTRIBUTION){
	 ...
	 for (i=0; i<SUBSIDIES_BALANCED.size; i++)
	    {
		add_redistributed_subsidies_message(SUBSIDIES_BALANCED.array[i].gov_id, SUBSIDIES_BALANCED.array[i].amount);
	    }
...
int Eurostat_calculate_data()
{
	...
	if(SUBSIDY_REDISTRIBUTION){
		...
		Eurostat_subsidies_redistribution();
