...
int Household_read_policy_announcements()
{
    ...
    if(policy_announcement_message->gov_id == GOV_ID)
    {
        ...
        BASIC_SECURITY_BENEFITS = policy_announcement_message->basic_security_benefits;
    }
...
int Household_receive_account_interest()
{
    ...
    MONTHLY_INTEREST_INCOME += interest;
...
{   
	...
	CONSUMPTION_BUDGET_IN_MONTH,PAYMENT_ACCOUNT,MEAN_NET_INCOME,WEALTH, HOUSEHOLD_INFLOWS_CALENDAR.total_dividends, HOUSEHOLD_INFLOWS_CALENDAR.deposit_interest , HOUSEHOLD_INFLOWS_CALENDAR.asset_sales - HOUSEHOLD_OUTFLOWS_CALENDAR.asset_purchases,WAGE_RESERVATION);
